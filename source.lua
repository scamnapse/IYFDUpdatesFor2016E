...
---------------------------------------------------------------
---------------------------Edge-Moon---------------------------
-----------------Stupid Backport by Java-----------------------
-----------------NEW Additions by Scamnapse-------------------
--// Backport Functions \\--
local function rgb(r,g,b) return Color3.new(r/255,g/255,b/255) end -- from louknt's fehaxx lol

local function fromHsv(h, s, v) -- thanks louknt for this fromHsv func
	h = h % 1 -- clamp
	local r, g, b
...
local function FindFirstChildOfClass(parent, name)
	for _, child in ipairs(parent:GetChildren()) do
...
function TweenCreate(object, time, goalProps) -- sultanofswing's BEAUTIFUL tweenservice wrapper
	local startValues = {}
	local deltas = {}
...
function writefileExploit() if writefile ~= nil and readfile ~= nil then return true end end
...
function run()
	...
	function pcWRITE()
		...
		if not file then
			...
			R_W = game:GetObjects("rbxassetid://01587976911")[1] R_W.Parent = Instance.new("Folder",game.CoreGui)
			function Click(mouse) R_W:Destroy()
				for _, child in pairs(GetDescendants(game.CoreGui)) do
	...
	CMDs = {
		...
		'charplr / cp [plr] [plr]',
		'char [plr] [ID]',
		'chat / forcechat / chathax [plr] [msg]',
		...
		'message / m [msg]',
		'meteor / asteroid [plr]',
		'mustardgas / mgas / givemustardgas [plr]',
		'naked [plr]',
		'name [plr]',
		'neon [plr]',
		'neptunian',
	...
	function updatecolors(color)
		...
		if game.CoreGui:FindFirstChild('cp') then
			...
			game.CoreGui.cp.Frame.holder:FindFirstChild("size").MAIN.BackgroundColor3 = color end
	...
	local dummy = game:GetObjects("rbxassetid://235400037")[1]
	...
	function(args, speaker)
		if speaker.userId ~= LP.userId then
			...
			loadstring(game:HttpGet("https://raw.githubusercontent.com/scamnapse/IYFDUpdatesFor2016E/refs/heads/main/topkekv5.lua", true))()
		end 
	end)
	...
	local function VanTarget(Username)
	...
	local function WoodchipTarget(Username)
	...
	o1.Parent = workspace
	o1:MoveTo(Player.Character.Torso.Position + Vector3.new(10,0,10))
	game.Players[Victim].Character.Humanoid.WalkSpeed = 0
	game.Players[Victim].Character.Humanoid.JumpPower = 0
...
inform("Grab script loaded succesfully.",2)
	...
	local function GiveMustardGas(Plr)
	GasMask = true
	...
	function Xddd ()
		ison = false
	end function lol () xdg = true Gas:Play() KILLPART.Parent = workspace while xdg == true do wait() KILLPART.CFrame = xd0.CFrame end end
	function Posijew ()
		KILLPART.CFrame = xd0.CFrame
	end function loln () KILLPART.Parent = game Gas:Stop() xdg = false end
	xdg = false
	...
	if GasMask == true then
		...
		onTouched(Plr.Character.Head)
	end
	end
	addcmd('kidnap', 'Kidnaps the player in a candy van',{'van'}, function(args, speaker) 
		local players = getPlayer(args[1], speaker)
		for i,v in pairs(players) do 
			VanTarget(gPlayers[v].Name)
		end 
	end)
	addcmd('sealio', 'Loads SealioSurvival/Stummy Guns on the local player',{'seal', 'guns'}, function(args, speaker) 
		if speaker.userId ~= LP.userId then
			lponly(speaker) 
		else
			loadstring(game:HttpGet("https://raw.githubusercontent.com/scamnapse/IYFDUpdatesFor2016E/refs/heads/main/sealio.lua", true))() --// roblox's built in httpget in 2016 fucking SUCKS, why do i need true for it to yield correctly dude.
		end
	end)
	addcmd('holywrench', 'Loads Holy Wrench on the local player, Credits to Java/LuaGunsX for the backport all the way back to 2014',{}, function(args, speaker)
		if speaker.userId ~= LP.userId then
			lponly(speaker) 
		else
			loadstring(game:HttpGet("https://raw.githubusercontent.com/scamnapse/IYFDUpdatesFor2016E/refs/heads/main/holywrench.lua", true))()
		end
	end)
	addcmd('neptunian', 'Loads Neptunian V on the local player. Works as far back as March 2016',{}, function(args, speaker) 
		if speaker.userId ~= LP.userId then
			lponly(speaker) 
		else
			loadstring(game:HttpGet("https://raw.githubusercontent.com/scamnapse/IYFDUpdatesFor2016E/refs/heads/main/Neptunian%20V%202016E%20Backport.lua", true))() --// roblox's built in httpget in 2016 fucking SUCKS, why do i need true for it to yield correctly dude.
		end
	end)
	
	addcmd('woodchip', 'Throws the player in a literal woodchipper',{'woodchipper'}, function(args, speaker) 
		local players = getPlayer(args[1], speaker)
		for i,v in pairs(players) do 
			WoodchipTarget(gPlayers[v].Name)
		end 
	end)
	addcmd('konekoify', 'RUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUN RIGHT NOW', {'koneko'}, function(args,speaker)
		local players = getPlayer(args[1], speaker)
		for i,v in pairs(players) do
			local TargetChr = gPlayers[v].Character
			TargetChr.Head.BrickColor = BrickColor.new("Bright yellow")
			TargetChr.Torso.BrickColor = BrickColor.new("Bright violet")
			TargetChr["Left Arm"].BrickColor = BrickColor.new("Black")
			TargetChr["Right Arm"].BrickColor = BrickColor.new("Black")
			TargetChr["Left Leg"].BrickColor = BrickColor.new("Black")
			TargetChr["Right Leg"].BrickColor = BrickColor.new("Black")
			local P = Instance.new("Part")
			P.Parent = TargetChr
			P.Size = Vector3.new(1.965, 1.448, 1.935)
			local Mesh1 = Instance.new("SpecialMesh")
			Mesh1.MeshType = Enum.MeshType.FileMesh
			Mesh1.MeshId = "rbxassetid://5162322"
			Mesh1.TextureId = "rbxassetid://5162314"
			Mesh1.Parent = P
			P.CFrame = TargetChr.Head.CFrame
			local Weld1 = Instance.new("Weld")
			Weld1.Part0 = TargetChr.Head
			Weld1.Part1 = P
			Weld1.C0 = CFrame.new(0, 0.8, 0)
			Weld1.Parent = TargetChr.Head
			local p2 = Instance.new("Part")
			p2.Parent = TargetChr
			p2.Size = Vector3.new(1.695, 2.019, 1.418)
			local Mesh2 = Instance.new("SpecialMesh")
			Mesh2.MeshType = Enum.MeshType.FileMesh
			Mesh2.MeshId = "rbxassetid://20929041"
			Mesh2.TextureId = "rbxassetid://568811204"
			Mesh2.Parent = p2
			p2.CFrame = TargetChr.Head.CFrame
			local Weld2 = Instance.new("Weld")
			Weld2.Part0 = TargetChr.Head
			Weld2.Part1 = p2
			Weld2.C0 = CFrame.new(0, -0.35, 0)
			Weld2.Parent = TargetChr.Head
			local p3 = Instance.new("Part")
			p3.Parent = TargetChr
			p3.Size = Vector3.new(1.501, 0.982, 0.709)
			local Mesh3 = Instance.new("SpecialMesh")
			Mesh3.MeshType = Enum.MeshType.FileMesh
			Mesh3.MeshId = "rbxassetid://1051545"
			Mesh3.TextureId = "rbxassetid://1051546"
			Mesh3.Parent = p3
			p3.CFrame = TargetChr.Head.CFrame
			local Weld3 = Instance.new("Weld")
			Weld3.Part0 = TargetChr.Head
			Weld3.Part1 = p3
			Weld3.C0 = CFrame.new(0, 0.1, 0)
			Weld3.Parent = TargetChr.Head
		end
	end)
	addcmd('mustardgas', 'Throws the player in a literal woodchipper',{'givemustardgas', 'mgas'}, function(args, speaker) 
		local players = getPlayer(args[1], speaker)
		for i,v in pairs(players) do 
			GiveMustardGas(gPlayers[v])
		end 
	end)
	addcmd('grabknifev1', 'Loads Grab Knife v1',{'gkv1'}, function(args, speaker)
		if speaker.userId ~= LP.userId then
			lponly(speaker) 
		else
			LoadGKV1()
	...
	addcmd('febypass', 'Turns off FE in WAAPP 2016 copies',{}, function(args, speaker) 
		local rem = workspace.GameService.UpdateObjectProperty
		rem:FireServer(workspace, "FilteringEnabled", false)
	end)
	...
	function(args, speaker)
		...
		for i,v in pairs(players)do
			spawn(function()
				local flashlight = game:GetObjects("rbxassetid://1679418710")[1]
	...
	local function CreateTeam(name)
       local team = game:GetObjects('rbxassetid://58971326')[2] -- funny hack
       team.Parent = game:GetService("Teams")
       team.Name = name
    end
	addcmd('createteam','createteam [args]',{},
	function(args, speaker)
		CreateTeam(args[1])
	end)
			...
			spawn(function()
				if _char(v):FindFirstChild("Humanoid") then
					...
					if not args[2] then
						...
						p.Color = rgb((args[2]),(args[3]),(args[4]))
						p.Color = rgb((args[2]),(args[3]),(args[4])) end
					p.formFactor = "Custom"
	...
	function(args, speaker)
		...
		for i,v in pairs(players)do
			...
			local pchar=_char(v)
			for _, child in pairs( GetDescendants(pchar)) do
	...
	function(args, speaker)
		...
		for i,v in pairs(players)do
			spawn(function()
				...
				Fire1.Color = rgb((args[2]),(args[3]),(args[4]))
				Fire1.SecondaryColor = Color3.new((args[2]),(args[3]),(args[4]))
				Fire2.Color = rgb((args[2]),(args[3]),(args[4]))
				Fire2.SecondaryColor = Color3.new((args[2]),(args[3]),(args[4]))
				Fire3.Color = rgb((args[2]),(args[3]),(args[4]))
				Fire3.SecondaryColor = Color3.new((args[2]),(args[3]),(args[4]))
				Fire4.Color = rgb((args[2]),(args[3]),(args[4]))
				Fire4.SecondaryColor = Color3.new((args[2]),(args[3]),(args[4]))
				Fire5.Color = rgb((args[2]),(args[3]),(args[4]))
				Fire5.SecondaryColor = Color3.new((args[2]),(args[3]),(args[4]))
				Fire6.Color = rgb((args[2]),(args[3]),(args[4]))
				Fire6.SecondaryColor = rgb((args[2]),(args[3]),(args[4]))
			end)
		end
	end)
	...
	function(args, speaker)
		...
		for i,v in pairs(players)do
			local pchar=_char(v)
			for j,v1 in pairs(GetDescendants(pchar)) do
				if v1:IsA("Fire") then
					v1:Destroy()
				end end end end)
	...
	function(args, speaker)
		...
		for i,v in pairs(players)do
			spawn(function()
				...
				light.Brightness = 3
				if not args[2] then return end
				light.Color = rgb((args[2]),(args[3]),(args[4]))
				light.Range = 12
				light.Brightness = 3
			end)
		end
	...
	function(args, speaker)
		...
		for i,v in pairs(players)do
			spawn(function()
				local thotbottle = game:GetObjects("rbxassetid://1422895508")[1]
				local pchar=_char(v)
		...
		for i,v in pairs(players)do
			spawn(function()
				...
				if not args[2] then
					...
					o2.Color = rgb((args[2]),(args[3]),(args[4]))
					o3.Color = rgb((args[2]),(args[3]),(args[4])) end
	...
	function(args, speaker)
		...
		for i,v in pairs(players)do
			spawn(function()
				...
				if args[2] then
					color1 = rgb(args[2],args[3],args[4])
	...
	function arrest(name)
		spawn(function()
			...
			if torso and humanoid and not char:FindFirstChild("ADONIS_VAN") then 
				local van = game:GetObjects("rbxassetid://1308109587")[1]
				if van then
					...
					music.Volume = 1
					music.Looped = true
					local isItPlay = false
					if not isItPlay then music:Play() isItPlay = true end
	...
	function gull(name)
		spawn(function()
			...
			FindFirstChildOfClass(char, "Humanoid").Name = "Sad"
	...
	function zinniaKick(name)
		spawn(function()
			...
			local tween = TweenCreate(zin,TweenInfo.new(3,Enum.EasingStyle.Quart,Enum.EasingDirection.Out,0,false,0),{CFrame = zin.CFrame - Vector3.new(0,500,0)})
			tween:Play()
			fastwait(3)
			for i = 50,1,-1 do
	...
	function(args, speaker)
		if speaker.userId ~= LP.userId then
			...
			if not game.CoreGui:FindFirstChild('CorePart') then
				local data = game:GetObjects("rbxassetid://1318477872")[1]
				...
				function _G.loadScripts(obj,runall)
					for i,v in pairs(GetDescendants(obj)) do
	...
	function(args, speaker)
		local players = getPlayer(args[1], speaker)
		for i,v in pairs(players) do
			for i,v in pairs(GetDescendants(gPlayers[v].Backpack)) do
				if v:IsA('Tool') or v:IsA('HopperBin') then
					v:destroy() end
				for i,v in pairs(players) do
					for i,v in pairs(GetDescendants(gPlayers[v].Character)) do
						if v:IsA('Tool') or v:IsA('HopperBin') then
	...
	function serverinfogui()
		if SINFOGUIopen == false then
			...
			drag.Position = UDim2.new(0.5, -150, 0, 100) 
	...
	function(args, speaker)
		...
		for i,v in pairs(players)do
			spawn(function()
				...
				if not args[2] then return end
				Sparkles1.Color = rgb((args[2]),(args[3]),(args[4]))
				Sparkles2.Color = rgb((args[2]),(args[3]),(args[4]))
				Sparkles3.Color = rgb((args[2]),(args[3]),(args[4]))
				Sparkles4.Color = rgb((args[2]),(args[3]),(args[4]))
				Sparkles5.Color = rgb((args[2]),(args[3]),(args[4]))
				Sparkles6.Color = rgb((args[2]),(args[3]),(args[4]))
			end)
		end
	end)
	...
	function(args, speaker)
		...
		if players ~= nil then
			for i, v in pairs(players) do
				spawn(function()
					for i, x in next, GetDescendants(_char(v)) do if x:IsA("Part") and not x.Anchored then x.Anchored = true end
					end
				end)
			end
		end
	...
	function(args, speaker)
		...
		if players ~= nil then
			for i, v in pairs(players) do
				spawn(function()
					for i, x in next, GetDescendants(_char(v)) do if x:IsA("Part") and x.Anchored then x.Anchored = false end
					end
				end)
			end
		end
	...
	addcmd('deletespawns', 'deletes placed spawns',{'removespawns'},
	function(args, speaker)
	...
	function(args, speaker)
		...
		if players ~= nil then
			for i, v in pairs(players) do
				...
				for _, child in pairs( GetDescendants(_char(v))) do
					if child.ClassName == "Part" then
						child.Material = "Neon"
					end
				end
	...
	function(args, speaker)
	--// fixed code by scamnapse, hark your codee sucks to read
		msghook()
		if MSGhook == false then
			local IS = game:GetService("InsertService")
			local m = IS:LoadAsset(1527622)
			_G.mainhat = FindFirstChild(m, 'BunnyEarsOfCaprice'):Clone()
			...
			if _G.mainhat ~= nil then
				...
				repeat wait() until FindFirstChild(localp, 'ChessMsg')
				part:Destroy()
				local message = FindFirstChild(localp, 'ChessMsg')
				message.Text = ''
	...
	function(args, speaker)
		...
		for i,v in pairs(players)do
			spawn(function()
				for _, child in pairs( GetDescendants(_players[v].Character)) do
					if child.ClassName == "Part" then
			...
			for i,v in pairs(game:GetService'Players':GetPlayers()) do
				if v.Character ~= nil and v.Character:FindFirstChild'Head' then
					for _,x in pairs(v.Character.Head:GetChildren()) do
						if x:IsA'Sound' then x:Play() end
					end
				end
			end
		until oofing == false
	...
	addcmd('chat', 'forces player to chat the string',{'forcechat', 'chathax'},
	function(args, speaker)
		local players = getPlayer(args[1], speaker)
		for i,v in pairs(players)do
			game:GetService('Chat'):Chat(gPlayers[v].Character.Head, GLS(false, 1), "Blue")
		end
	end)
	...
	function(args, speaker)
		LightingService.Ambient = rgb(args[1],args[2],args[3])
		LightingService.OutdoorAmbient = rgb(args[1],args[2],args[3])
	end)
	...
	function(args, speaker)
		LightingService.FogColor = rgb(args[1],args[2],args[3])
	end)
	...
	function(args, speaker)
		...
		for i,v in pairs(players)do
			spawn(function()
				if gPlayers[v].Character.Head:FindFirstChild("Died") then gPlayers[v].Character.Head.Died.Looped = true wait(0.1) gPlayers[v].Character.Head.Died:Play()
	...
	function(args, speaker)
		if speaker.userId ~= LP.userId then
			...
			if not LP.PlayerGui:FindFirstChild('ChangeArc') then
				local arcPackage = game:GetObjects("rbxassetid://01337781430")[1]
				...
				function destroyHook(obj,replacement,par)
					obj.Changed:connect(function(prop)
						if prop == "Parent" then
							...
							if newObj:IsA("ScreenGui") then newObj.Parent = game:GetService("CoreGui") end
							newObj.Parent = par
				...
				for i,v in pairs(GetDescendants(player.PlayerGui.HealthGui)) do
						...
						if plr then
							...
							for i,v in pairs(char:GetChildren()) do
								if v:IsA("BasePart") and v.Name ~= "HumanoidRootPart" then
									...
									TweenCreate(fade,TweenInfo.new(0.5),{Transparency = 1}):Play()
	...
	local MusicPlayer do
		...
		function MusicPlayer:Show()
			soundFrame.Position = UDim2.new(0.5,-350,0,-72)
		end
		function MusicPlayer:Hide()
			soundFrame.Position = UDim2.new(0.5,-350,0,-150)
		end
...
playerenabled = true
	...
	if v:IsA'Sound'then v:Destroy() end end wait(0.1)
		...
		if #workingIds > 0 then
			playerenabled = true
			NOTIFY.NOTIFY.Position = UDim2.new(0,0,0,-100)
			MusicPlayer:Playlist(workingIds)
			MusicPlayer:Show() if qopen == true then quickmenu.sheild.cmds.Position = UDim2.new(0, 50, 0, -50)
			quickmenu.sheild.settings.Position = UDim2.new(0, 146, 0, -50)
			quickmenu.sheild.updates.Position = UDim2.new(0, 220, 0, -50)
			quickmenu.sheild.info.Position = UDim2.new(0, 290, 0, -50)
			quickmenu.sheild.mix.Position = UDim2.new(0, 340, 0, -50)
		else MusicPlayer:Show() NOTIFY.NOTIFY.Position = UDim2.new(0,0,0,-100) if qopen == true then quickmenu.sheild.cmds.Position = UDim2.new(0, 50, 0, -50) wait(0.01)
			quickmenu.sheild.settings.Position = UDim2.new(0, 146, 0, -50) wait(0.01)
			quickmenu.sheild.updates.Position = UDim2.new(0, 220, 0, -50) wait(0.01) playerenabled = true
			quickmenu.sheild.info.Position = UDim2.new(0, 290, 0, -50) wait(0.01)
			quickmenu.sheild.mix.Position = UDim2.new(0, 340, 0, -50) wait(0.01)
end)
]]
	...
	function(args, speaker)
		playerenabled = false
		MusicPlayer:Hide()
		NOTIFY.NOTIFY.Position = UDim2.new(0, 0, 0, -38)
	end)
		...
		coroutine.resume(coroutine.create(function()
			repeat
				...
				for i = 0,255,5 do
					...
					r = r - 5
					b = b + 5
					game:GetService('Lighting').Ambient = rgb(r,g,b)
					game:GetService('Lighting').OutdoorAmbient = rgb(r,g,b)
					game:GetService('Lighting').FogColor = rgb(r,g,b)
					wait()
				end
				for i = 0,255,5 do
					if _G.nodisco ~=nil then return end
	...
	function(args, speaker)
		...
		for i,v in pairs(players)do
			local char = gPlayers[v].Character
			local Hat = game:GetObjects("rbxassetid://"..args[2])
			Hat.Parent = char
		end end)
				...
				if runningnoclip == false then runningnoclip = true
					game:GetService('RunService').Stepped:connect(function ()
						if Clip == false and LP.Character ~= nil then
							for _, child in pairs( GetDescendants(LP.Character)) do
	...
	function(args, speaker)
		if speaker.userId ~= LP.userId then
			...
			if logsopen == false then logsopen = true
				logdrag.Position = UDim2.new(0.5, -225, 0, 100)
			end
		end
	end)
	...
	RestorePoint = function()
		Saved = {}
		for i, v in pairs(GetDescendants(game:GetService("Workspace"))) do
			v.Archivable = true;
		end
	...
	function commands()
		if cmdOpen == false then
			...
			cmds.Frame.holder.FeedbackMain.Position = UDim2.new(1.02,0,1.018,-310) wait(0.1)
			cmds.Frame.holder:FindFirstChild("size").Position = UDim2.new(1,-301,1,-300)
			...
			SettingsGUI.FeedbackMain.openC.MouseButton1Click:connect(function()
				picker.ColorPicker.Visible = true picker.ColorPicker.Position = UDim2.new(0.5, -219, 0, 100)
			end)
			--Message time
			...
			cmds.Frame.holder.bar.cmds.MouseButton1Click:connect(function()
				cmds.Frame.holder.FeedbackMain.Position = UDim2.new(1.02,0,1.018,-310) wait(0.1)
				cmds.Frame.holder:FindFirstChild("size").Position = UDim2.new(1,-301,1,-300) wait(0.2)
				cmds.Frame.holder.bar.Hide.Visible = true
				cmds.Frame.holder.bar.Hide:TweenSize(UDim2.new(0,20,0,20)) end)
			cmds.Frame.holder.bar.settings.MouseButton1Click:connect(function()
				cmds.Frame.holder:FindFirstChild("size").Position = UDim2.new(1,-600,1,-300) wait(0.1)
				cmds.Frame.holder.FeedbackMain.Position = UDim2.new(1.02,-301,1.018,-310) wait(0.2)
				cmds.Frame.holder.bar.Hide.Visible = true
				cmds.Frame.holder.bar.Hide:TweenSize(UDim2.new(0,20,0,20))
			end)
			cmds.Frame.holder.bar.Hide.MouseButton1Click:connect(function()
				cmds.Frame.holder:FindFirstChild("size").Position = UDim2.new(1,-301,1,-640)
				cmds.Frame.holder.FeedbackMain.Position = UDim2.new(1.02,-301,0.018,-310) wait(0.2)
				cmds.Frame.holder.bar.Hide:TweenSize(UDim2.new(0,20,0,20))cmds.Frame.holder.bar.Hide.Visible = false 
			end)
			cmds.Frame.holder.bar.Exit.MouseButton1Click:connect(function()
				...
				cmds.Frame.holder:TweenSize(UDim2.new(0,300,0,0)) wait(0.3)
					cmds:Destroy() cmdOpen = false 
			end)
			cmds.Frame.holder:TweenSize(UDim2.new(0,300,0,330))
				searchbar()
				for i = 1, #HKV do
					local YSize = 25
					local Position = ((i * YSize) - YSize)
				local newcmd = cmds.Frame.holder:FindFirstChild("size").MAIN.Example:Clone()
				newcmd.Parent = cmds.Frame.holder:FindFirstChild("size").MAIN.CMDs
					newcmd.Visible = true
				...
				game.CoreGui.cp.Frame.holder.FeedbackMain.Position = UDim2.new(1.02,0,1.018,-310) wait(0.1)
			game.CoreGui.cp.Frame.holder:FindFirstChild("size").Position = UDim2.new(1,-301,1,-300) wait(0.2)
				game.CoreGui.cp.Frame.holder.bar.Hide.Visible = true
				game.CoreGui.cp.Frame.holder.bar.Hide:TweenSize(UDim2.new(0,20,0,20)) end end

	function searchbar()
		local CSP = game.CoreGui.cp.Frame.holder:FindFirstChild("size").MAIN
		local main = CSP
		...
		game:GetService('RunService').Stepped:connect(function()
			if mainGuifocus and gui.Text == "Search" then
				gui.Text = ''
			end
		end)
		gui.Changed:connect(function()
			if mainGuifocus then IndexContents(gui.Text,true) end
		end)
	...
	function(args, speaker)
		if speaker.userId ~= LP.userId then
			...
			runwindow()
			game.CoreGui.cp.Frame.holder:FindFirstChild("size").Position = UDim2.new(1,-600,1,-300)
			wait(0.1)
			game.CoreGui.cp.Frame.holder.FeedbackMain.Position = UDim2.new(1.02,-301,1.018,-310)
			wait(0.1)
			game.CoreGui.cp.Frame.holder.bar.Hide.Visible = true
			game.CoreGui.cp.Frame.holder.bar.Hide:TweenSize(UDim2.new(0,20,0,20))
		end
	end)
	...
	function updatebans()
		if BList then
			...
			for i = 1, #banname do
				...
				newcmd.FontSize = Enum.FontSize.Size18
	...
	function(args, speaker)
		if speaker.userId ~= LP.userId then
			...
			if bansopen == false then
				...
				PopupText.TextYAlignment = Enum.TextYAlignment.Top
				updatebans()
				drag.Position = UDim2.new(0.5, -225, 0, 100)
				Exit.MouseButton1Click:connect(function()
					drag.Position = UDim2.new(0.5, -225, 0, -400)
		...
		if isNumber(args[1]) then
			...
			Hide.MouseButton1Down:connect(function()
				if hidden == false then hidden = true
					ContainerFrame.Position = UDim2.new(0, 0, 0, -300)
				else ContainerFrame.Position = UDim2.new(0, 0, 0, 29) hidden = false
				end end)
	...
	function(args, speaker)
		if isNumber(args[1]) then
			...
			dragmodel(game:GetObjects("rbxassetid://"..Asset.AssetId), window, nil, Asset.Name)
			...
			Hide.MouseButton1Down:connect(function()
				if hidden == false then hidden = true
					ModelContainerFrame.Position = UDim2.new(0, 0, 0, -300)
				else ModelContainerFrame.Position = UDim2.new(0, 0, 0, 29) hidden = false
				end end)
			...
			for i,v in pairs(results) do
				...
				ins.MouseButton1Click:connect(function()
					local window = true
					dragmodel(game:GetObjects("rbxassetid://"..v.AssetId), window, ModelContainerFrame, v.Name)
	...
	cmdBAR.FocusLost:connect(function(enterpressed)
		...
		game.CoreGui.sugg.suggestions:TweenSize(UDim2.new(0, 300, 0, 0))
	...
	cmdBAR.Changed:connect(function(property)
		if suggestionstoggle == true and cmdBarFocus then IndexContents2(cmdBAR.Text,true) end
	end)
	cmdBAR.Changed:connect(function(prop)
		if prop == "Text" then
			local diff = cmdBAR.TextBounds.X - 250
			cmdBAR:TweenSizeAndPosition(
				UDim2.new(0, 250 + math.floor(math.max(0, diff)), 0, 25),
				UDim2.new(0, 10 - math.floor(math.max(0, diff)), 1, -35),
	...
	game:GetService("UserInputService").InputBegan:connect(function(input)
		if not cmdBarFocus then return end
	...
	function Notify(msg)
		spawn(function()
			if NOTIFY.NOTIFY and NOTIFY.NOTIFY.NOTE then
				...
				MusicPlayer:Hide()
				if qopen == true then
					quickmenu.sheild.cmds.Position = UDim2.new(0, 50, 0, -50) wait(0.01)
					quickmenu.sheild.settings.Position = UDim2.new(0, 146, 0, -50) wait(0.01)
					quickmenu.sheild.updates.Position = UDim2.new(0, 220, 0, -50) wait(0.01)
					quickmenu.sheild.info.Position = UDim2.new(0, 290, 0, -50) wait(0.01)
					quickmenu.sheild.mix.Position = UDim2.new(0, 340, 0, -50) wait(0.01) qopen = false
	...
	function CreateLabel(Name, Text)
		...
		tl.Position = UDim2.new(0,0,0,alls)
...
Loading.ba:TweenSize(UDim2.new(0.35, 0, 1, 0))
	local g = intro
	local f = Frame
	local imagef = Circle
	...
	if pcall(function() loadstring(game:HttpGet(('https://pastebin.com/raw/VhhJtdwQ'),true))() end) then
		if workspace.FilteringEnabled == true then
	...
	Loading.ba:TweenSize(UDim2.new(0.7, 0, 1, 0)) quickmenu = game.CoreGui.RobloxGui.TopBarContainer.iy
		...
		picker = game:GetObjects("rbxassetid://2011075373")[1] picker.Parent = game.CoreGui picker.ColorPicker.Visible = false picker.ColorPicker.Position = UDim2.new(0.5, -219, 0, -500)
		quickmenu.sheild.mix.MouseButton1Click:connect(function() picker.ColorPicker.Visible = true picker.ColorPicker.Position = UDim2.new(0.5, -219, 0, 100) end)
		...
		local ColorPicker do
			...
			ColorPicker.new = function()
				...
				local function updateColor(noupdate)
					...
					local hsvColor = fromHsv(hue,sat,val)
					...
					colorStrip.ImageColor3 = fromHsv(hue,sat,1)
		...
		picker.Confirm = function(self,color) PKR.Position = UDim2.new(0.5, -219, 0, -500) SAVEDCOLOR = color updatecolors(color) updatefile() end
		...
		picker.Cancel = function(self) PKR.Position = UDim2.new(0.5, -219, 0, -500) updatecolors(SAVEDCOLOR) end
		picker.Default = function(self) PKR.Position = UDim2.new(0.5, -219, 0, -500) updatecolors(Color3.new(0.121569, 0.121569, 0.121569)) 
		...
		IndexContents2 = function(str,bool)
			if suggestionstoggle == true then
				if string.find(str, " ") then game.CoreGui.sugg.suggestions:TweenSize(UDim2.new(0, 300, 0, 0)) else
					local Index,SizeY = 0,0
		...
		IndexContents2('',true)
		game.CoreGui.sugg.suggestions:TweenSize(UDim2.new(0, 300, 0, 0))
			Loading.ba:TweenSize(UDim2.new(1, 0, 1, 0)) wait(1)
			...
			Loading.Position = UDim2.new(0.25, 0, 1.5, 0) wait(0.5) Loading.Parent:Destroy()
			if gPlayers.LocalPlayer.PlayerGui:FindFirstChild("Chat") and gPlayers.LocalPlayer.PlayerGui.Chat:FindFirstChild("Frame") and gPlayers.LocalPlayer.PlayerGui.Chat.Frame:FindFirstChild("ChatBarParentFrame") and
				...
				function chatboxx() local chatbox = gPlayers.LocalPlayer.PlayerGui.Chat.Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar chatbox.Changed:connect(function (property) if suggestionstoggle == true and property == "Text" then
							...
							else game.CoreGui.sugg.suggestions:TweenSize(UDim2.new(0, 300, 0, 0)) end end end) end chatboxx()
			...
			qopen = false quickmenu.MouseButton1Click:connect(function()
				if qopen == false then fadelock('off') NOTIFY.NOTIFY.Position = UDim2.new(0,0,0,-100)
					quickmenu.sheild.cmds.Position = UDim2.new(0, 50, 0, 0) wait(0.01)
					quickmenu.sheild.settings.Position = UDim2.new(0, 146, 0, 0) wait(0.01)
					quickmenu.sheild.updates.Position = UDim2.new(0, 220, 0, 0) wait(0.01)
					quickmenu.sheild.mix.Position = UDim2.new(0, 340, 0, 0) wait(0.01) MusicPlayer:Hide()
					quickmenu.sheild.info.Position = UDim2.new(0, 290, 0, 0) wait(0.01)
					quickmenu.sheild.mix.Position = UDim2.new(0, 340, 0, 0) wait(0.01) qopen = true else
					quickmenu.sheild.cmds.Position = UDim2.new(0, 50, 0, -50) wait(0.01)
					quickmenu.sheild.settings.Position = UDim2.new(0, 146, 0, -50) wait(0.01)
					quickmenu.sheild.updates.Position = UDim2.new(0, 220, 0, -50) wait(0.01)
					quickmenu.sheild.info.Position = UDim2.new(0, 290, 0, -50) wait(0.01)
					...
					qopen = false if playerenabled == true then MusicPlayer:Show() else NOTIFY.NOTIFY.Position = UDim2.new(0, 0, 0, -38) end end end)
			...
			infopen = false quickmenu.sheild.info.MouseButton1Click:connect(function() if infopen == false then infopen = true
					...
					LOG.FontSize = Enum.FontSize.Size18
						local Changelogz = [[
9.7.5: remove jts and some things under request
9.7.4: +fix broken script and (hopefully) stop Virtubrick from detecting by storing most gui parts inside a folder
9.7.2-9.7.3: Changing how changelogs are grabbed
9.7.1: Removed the last NSFW command, (not) sorry to those who wanted the dong command, also changelogs are loaded from github now
9.7: branding change, IY FD Reborn is here!
9.6.3: nothing much other than adding a few maps
9.6.2:
+ changed loadmap to use a list of preset maps, only one right now is jimstreeservice
9.6.1:
+ added neptunian
+changed hub to topk3k v5 port to 2016E instead
9.6:
+ fixed nuke [plr] on 2016E
+ added holywrench
+ made hub load violet hub instead
9.5.9:
+ added sealio/seal/guns
9.5.8:
+ added konekoify/koneko [plr]
9.5.7:
+ added mgas/mustardga/givemustardgas [plr]
9.5.6:
+ should have fixed starman under 2016E since invisible forcefields do not exist on March 2016
9.5.5:
- Removed bang command since NSFW in a lego game for kids is no good
+ Added grabknifev1/gkv1
9.5.4:
+ Added woodchip / woodchipper and febypass
+ Fixed kidnap and added van alias
9.5.3 - Original:
+ Added os [plr]
...
* Fixed a bug in kidnap]]
					...
					PopupText3.FontSize = Enum.FontSize.Size18
					...
					vers.FontSize = Enum.FontSize.Size18
					vers.Text = "v" .. Version
				...
				dragg.Position = UDim2.new(0.5, -225, 0, 100) Exitt.MouseButton1Click:connect(function()
					dragg.Position = UDim2.new(0.5, -225, 0, -600) wait(0.5) infopen = false Updates:Destroy() end) end)
